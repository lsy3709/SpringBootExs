...
//        System.out.println("savedOrder.getOrderItems().size()======"+ savedOrder.getOrderItems().size());
    ...
    public void orphanRemovalTest(){
        ...
        System.out.println("고아객체 제거 전 테스트1 order.getOrderItems().size(): "+ order.getOrderItems().size());
        order.getOrderItems().remove(0);
        System.out.println("고아객체 제거 후 테스트2 order.getOrderItems().size(): "+ order.getOrderItems().size());
        // 메모리 상에 리스에서 제거만 하고, 디비 상에 반영을 안해서, 동작을 제대로 
        // 못했음. 그래서 아래에 디비 반영 추가하기. 
        orderRepository.save(order);
        
        // 수동으로 디비에 반영하는 명령어 인데. 
